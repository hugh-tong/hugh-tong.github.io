...
> 补充：Kundu, *Fluid Mechanics*, 5th ed., p.86 用移动积分边界示意图解释了莱布尼茨法则。这里保留书目信息，不嵌入教材页面截图。
...
考虑一个二维矩形控制体：各边界以不同网格速度移动，所以新旧时间层的控制体面积不同。
...
令 $\delta V_k$ 表示第 $k$ 个控制体面在两个时间层之间扫过的有符号体积。
