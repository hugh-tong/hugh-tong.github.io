...
tags:
  ...
  - 求解器
  - 求解器
description: ALE 动网格框架下推导满足 Spatial Conservation Law 的输运方程完整过程。
mathjax: true
---
...
> 补充，对于莱布尼茨法则，Kundu的书（Fluid Mechanics-5th edition, P86）中对这个法则有直观的图表示：
>
> ![image-20240227164531091](https://testnew-1307662376.cos.ap-chengdu.myqcloud.com/image-20240227164531091.png)
...
补充：
$$
\text{\bf{散度定理}:}\quad\int_S \vec{v}\cdot \vec{n}dS= \int_V{\nabla \cdot \vec{v}}~dV
$$
...
2020-3-1，补充：
$$
\frac{dV}{dt} = \int_{\partial V_p}\vec{v_s}\cdot d{\vec{S}} = \sum_f \vec{v_s}\cdot \vec{S_f} \\
\sum_f \vec{v_s}\cdot \vec{S_f} = \frac{\delta V_f}{\Delta t} \\
\vec{S} = \vec{n}dS \\
\delta V_f: \text{ Volume swept out by the control volume face f over time step} 
$$

Note: To avoid stability and accuracy problems due to cell quality(as the cells are changing in time), the time step should be chosen in such a way that it does not result in large volume changes between iterations.
